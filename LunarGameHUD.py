...
class LunarGameHUD():
    def __init__(self, grid_or_game):
        ...
        self.warn_out_of_bounds.setFont(hud_font)
        ...
        try: # versuche, aus dem geg. Grid/Game Werte zu holen 
            ...
            if hasattr(self.grid_or_game, 'out_of_bounds_timer'):
                if self.grid_or_game.out_of_bounds_timer // 100 < 50:
                    if self.warn_out_of_bounds.getTextActor().isVisible(): self.warn_out_of_bounds.hide()
                else:
                    if not self.warn_out_of_bounds.getTextActor().isVisible(): self.warn_out_of_bounds.show()
            else:
                if self.warn_out_of_bounds.getTextActor().isVisible(): self.warn_out_of_bounds.hide()
                """
